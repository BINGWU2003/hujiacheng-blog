...
_用于浏览器在存在多条`CSS`规则指向同一元素时，确定哪条规则的声明应被最终应用_
  ...
  <body>
    ...
    <div class="comparison">
      <p class="comparison">
        ...
        <br />
        默认情况下，第一行受 <code>p.comparison::first-line</code> (特指度
        0,1,2) 控制，是<b>蓝色</b>的。
        <br />
        当你悬停时，<code>p.comparison:hover::first-line</code> (特指度 0,2,2)
        将会生效。
        <br />
        观察第一行的颜色变化。
      </p>
    ...
    <h2>分析</h2>
    <p>
      在这个修正的例子中，两个规则都**直接**以
      <code>::first-line</code> 为目标。
    </p>
    <ul>
      <li>
        规则 1 (默认): <code>p.comparison::first-line</code> { color: blue; } -
        <b>特指度 (0, 1, 2)</b>
      </li>
      <li>
        规则 2 (悬停): <code>p.comparison:hover::first-line</code> { color: red;
        } - <b>特指度 (0, 2, 2)</b>
      </li>
    </ul>
    <p>
      当鼠标悬停时，两个规则都匹配，浏览器比较 <b>(0, 1, 2)</b> 和
      <b>(0, 2, 2)</b>：
    </p>
    <ol>
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <p>
      ...
      但是，第一行的优先级被 <code>p::first-line</code> 选择器（特指度
      0,0,2）覆盖了， 所以第一行应该是蓝色的，并且是粗体。
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <p>
      ...
      <br />
      <b>特指度平局！</b> 规则 B 在 CSS 中定义在最后，所以规则 B 获胜，颜色保持
      <code>green</code>。
    </p>
    ...
    <p>
      ...
      <br />
      <b>特指度平局！</b> 规则 D 在 CSS 中定义在最后，所以规则 D 获胜，颜色变为
      <code>blue</code>。
    </p>
  </body>
</html>
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <ul>
      <li>
        规则 1 (ID): <code>#element-id</code> { color: red; } -
        <b>特指度 (1, 0, 0)</b>
      </li>
      <li>
        规则 2 (B类组合): <code>.element-class[data-role="test"]:hover</code> {
        color: blue; } - <b>特指度 (0, 3, 0)</b>
      </li>
    </ul>
    <p>浏览器比较 <b>(1, 0, 0)</b> 和 <b>(0, 3, 0)</b>：</p>
    <ol>
    ...
    <p>
      ...
      <br />
      这就是 CSS 特指度的“不可进位”原则：即使我们堆叠了 3 个（甚至 99 个）B
      类选择器，它们的权重也永远无法超过 1 个 A 类（ID）选择器。
    </p>
  </body>
</html>
```
