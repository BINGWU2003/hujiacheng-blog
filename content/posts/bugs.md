...
今天按照我自己文章统一代码风格和规范项目代码搭建项目时，走到配置 stylelint 这步时出现了问题:
...
module.exports = {
  extends: [
    ...
    // 'stylelint-config-prettier', // v15已废弃
...
其实问题原因是在于你的`eslint`文件的格式，当你的`eslint`版本大于等于`v8.21`时，且文件格式为`eslint.config.js`就会出现这种问题，这时候只需要打开 vscode 的相关配置即可。
...
不管是新的还是旧的 ios，它们的 safari 浏览器的屏幕高度(screen height)都是包括了工具栏和地址栏的。网页会被地址栏工具栏遮挡掉一部分，而且上下滑动也无法解决。而安卓的屏幕高度并不会包括工具栏地址栏。
...
这个 bug 是因为 checkbox 选中后伪元素的 animation 动画放大导致的。
...
4. checkbox纵向排列时，只有最后一个checkbox选中才会触发bug

```css
.ant-checkbox-checked::after {
  /* ... */
  animation: antCheckboxEffect 0.36s ease-in-out;
  /* 禁用或修改动画scale使效果扩大变小一点即可解决 */
}
```
